...
public sealed class KafkaConsumerHostedService : BackgroundService
{
    ...
    {
        _consumer.Subscribe(Constants.KafkaTopics.KeywordIndexing);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var consumer = scope.ServiceProvider.GetRequiredService<IKeywordIndexingConsumer>();
            ...
            {
                _logger.LogError(ex, "Error consuming message from Kafka topic '{Topic}'", Constants.KafkaTopics.KeywordIndexing);
            }
            catch (KafkaException ex)
            {
                ...
                _logger.LogError(ex, "Kafka broker error while processing/committing on topic '{Topic}'; message will be redelivered.", Constants.KafkaTopics.KeywordIndexing);
            }
        }
    }
